...
分路由下限和合格样本均分后，自动进入一次 30 条全新任务的发布留出集：
...
留出集固定采用独立 seed、从零构建和 live rollout。它要求 30 个任务均为实际生成且内容、seed
不与开发轮重复，端到端合格率不低于 70%；每个合格沙箱的 3 次 rollout 至少成功 2 次，
所有已执行 episode 都不得出现环境错误或 LLM fallback。留出集只执行一次，失败即保留证据并退出，
不能回流调参后继续冒充留出集。
...
- `--rollout-episodes 3 --rollout-steps 20` 控制每个沙箱的轨迹次数和步数。开发阶段默认只要求至少
  一条成功证据；`--rollout-min-success-rate` 可提高该门槛。发布留出集默认使用
...
- `--max-total-seconds` 默认 21600，只累计实验进程的活跃执行时间；正常暂停不消耗预算。`runtime_state.json` 保存累计活跃时间和运行状态。质量循环不再因“停滞”或固定 20 轮提前结束；显式设置正数 `--max-rounds` 才启用轮数预算。基础设施超时属于异常中止而非质量收敛。
...
不允许在运行中的实验目录对应源码上继续修改。最终候选冻结后由调度器自动运行 30 个全新留出任务，
`history.json` 的最终停止原因只有 `holdout_target_met` 才表示完整实验通过。
