...
7. Expose the sandbox to an Agent through a local HTTP API and generate `tools.json` for the RL Trainer. It must use the standard function-tool shape: each tool has `name`, `description`, and `input_schema` (`type: object`, `properties`, `required`, and `additionalProperties: false`). Describe parameter types, meanings, constraints, and examples in the JSON Schema. Include transport, returns, and visibility as additional metadata. Include every task action plus `ask_user`, `reset`, `get_observation`, and `get_reward`.
...
- Do not copy an empty or generic parameter schema from the task input. Derive and implement the real action inputs in code, tests, and `tools.json`.
