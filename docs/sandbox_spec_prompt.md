...
The sandbox must include `requirements-dev.txt` with an exact `pytest==...` pin and exact pins for every directly installed test dependency, install it in the Docker image, and run `python3 -m pytest -q`. Version ranges or unpinned dependencies are not production reproducibility evidence. A missing pytest installation is a failed build, not a passed or silently skipped test. After acceptance, write `acceptance_result.json` with `business_acceptance: "passed"` and `http_conformance: "passed"` or `"skipped"`; if HTTP is skipped, include a non-empty `http_skip_reason`.
