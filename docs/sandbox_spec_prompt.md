...
Mandatory step-type rule: the only allowed `action_plan.json` step kinds are `public_llm_tool` and `llm_generate`; do not use `internal`. Every atomic action must be represented by a `public_llm_tool` step and exposed in `llm_tools`; validation, normalization, transaction, persistence, and hidden evaluation belong in the implementation details of that public action. The preceding generic wording about `internal` is superseded by this rule.
